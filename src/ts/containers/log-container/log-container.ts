...
class LogContainer {
    ...
    checkIfUserInputValuesFormatValid (email: string, password:string): boolean {
        ...
        if(!isEmailFormatValid){
            document.getElementById("email-error").innerHTML = `<p>Email incorrect</p>`
            return
        } else {
            document.getElementById("email-error").innerHTML = "" 
        }

        const isPasswordFormatValid = this.formatCheckService.checkPassword(password)   
        if(!isPasswordFormatValid){
            document.getElementById("password-error").innerHTML = `<p>Le mot de passe doit contenir au minimum 12 caractères et au maximum 20</p>`
            return
        }  else {
    ...
    onSubmit(event: any){
       ...
       if(result){
            const isFormatOk = this.checkIfUserInputValuesFormatValid(result.emailValue, result.passwordValue)
            if(isFormatOk){
                this.notificationService.notif = {
                    type: "success",
